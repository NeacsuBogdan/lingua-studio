...
Phase 0 and Phase 1 are complete. Phase 2 code introduces real account/session storage and owner-only routes. Live GitHub OAuth remains an external validation gate. The supplied master brief is retained in docs/MASTER-BRIEF.txt. No lesson execution, progress, scores, or exam claims are fabricated.
...
The application pages require a server-validated owner session. The public sign-in page gives a setup state when OAuth is incomplete; there is no mock login. The profile server action derives the user ID from that session, validates input with Zod and writes only that user's row. The database migration runs explicitly and is applied on the Neon development database. The example lesson is a schema fixture, not an available course. Do not claim CEFR certification or official Cambridge endorsement. Verify Cambridge sources in Phase 16.
