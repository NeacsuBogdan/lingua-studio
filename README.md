...
Exercise answers are graded against persisted content on the server. Hidden correct-answer fields and explanations are omitted before submission. Text normalization preserves accents and internal punctuation; whitespace, case and final punctuation follow authored rules. Migration `0004` adds append-only attempt history. Lesson versions rise to 2: seeding preserves existing profiles/progress, and a revised lesson starts afresh when reopened. No vocabulary mastery, spaced repetition, mistake system or AI grading is included.
